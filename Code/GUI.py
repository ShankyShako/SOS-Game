...
def CreateGame(GUI, brd_int, GL, CO):
    ...
    gmc = partial(GL.GameModeChange, GUI, g, b_p, r_p)
    rdbtn1 = Radiobutton(GUI,
                          text='Simple game', command=gmc, variable = g,
                         value = "Simple game")
    rdbtn1.place(x=board_s_x/2-5, y=10, anchor=NE)
    
    rdbtn2 = Radiobutton(GUI,
                          text='General game', command=gmc, variable = g,
                         value = "General game")
    ...
    board_size_text = Label(GUI,
                          text = "Board size")
    board_size_text.place(x = board_s_x - 110,
                                                   y = 10)
    ...
    red_p_text = Label(GUI,
                          text = "Red player")
    red_p_text.place(x = 250 + board_int.get()* 24,
                                                   y = 50)
    ...
    rb = partial(ResizeBoard, GL, GUI, board_size, 15)
    
    GUIArray = [b_p, r_p, rdbtn1, rdbtn2, board_size_text, board_size, blue_p_text, blue_s_btn, blue_o_btn, red_p_text, red_s_btn, red_o_btn, current_turn_text, button, rec, rb, g]
    ...
    org = partial(GL.Rec.OpenRecPrompt, GUI, GUIArray)
    sg = partial(GL.Rec.SaveReplay)
    ...
    rec.add_command(label ='Save Game', command = sg)
    #red.add_command(label ='Replay Game', command = None)
