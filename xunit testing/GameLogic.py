...
from Simple_Game import Simple
from General_Game import General
...
class GameBoard():
    ...
    def NewGame(self, GUI, button):
        ...
        if self.curr_gamemode == self.gen_game:
            self.b_p.config(text="Points: 0")
            self.r_p.config(text="Points: 0")
        self.current_player = self.b_player
        self.isOver = False
        if self.current_player.get() == "C":
            self.computer_player.PlayMove(GUI, button,  self.c_t, self.current_turn_text, self.b_p, self.r_p)
    
    def NewSimpleGame(self, GUI, option, b_p, r_p):
        c_t = StringVar()
        option.set("Simple game")
        self.curr_gamemode = self.sim_game
        self.b_p.config(text="")
        self.r_p.config(text="")
        self.prompt_up = False
        self.ng_or_oc = 1
        ...
        if option.get() != self.curr_gamemode.name:
            if self.curr_gamemode.name == "Simple game":
                if self.is_point == True and self.turn < len(self.board[0]) * len(self.board) - 1:
                    ...
                    self.b_p.config(text="Points: " + str(self.blue_points))
                    self.r_p.config(text="Points: " + str(self.red_points))
                    self.isOver = False
    ...
    def CheckPoint(self, GUI, x, y, c_t, button):
        ...
        if self.board[x][y] == "S":
            ...
            if x+1 <= x_bound and self.board[x+1][y] == "O" and x+2 <= x_bound and self.board[x+2][y] == "S":
                #self.canvas.create_line(x * 24,  y*26, (x+2) * 24,  y*26, fill = c_t.get().lower(), width = 5)
                ...
                #self.canvas.create_line(x * 24,  y*26, (x+2) * 24,  (y+2)*26, fill = c_t.get().lower(), width = 5)
                button[x][y].config(bg=w_color)
                ...
                #self.canvas.create_line(x * 24,  y*26, x * 24,  (y+2)*26, fill = c_t.get().lower(), width = 5)
                button[x][y].config(bg=w_color)
            ...
            if x+1 <= x_bound and self.board[x+1][y] == "S" and x-1 >= 0 and self.board[x-1][y] == "S":
                #self.canvas.create_line((x-1) * 24, y*26, (x+1) * 24, y*26, fill = c_t.get().lower(), width = 5)
                button[x][y].config(bg=w_color)
                button[x+1][y].config(bg=w_color)
                button[x-1][y].config(bg=w_color)
                return True
            elif y+1 <= y_bound and self.board[x][y+1] == "S" and y-1 >= 0 and self.board[x][y-1] == "S":
                #self.canvas.create_line(x * 24, (y-1)*26, x * 24, (y+1)*26, fill = c_t.get().lower(), width = 5)
                button[x][y].config(bg=w_color)
                button[x][y+1].config(bg=w_color)
                button[x][y-1].config(bg=w_color)
                return True
            elif x+1 <= x_bound and y+1 <= y_bound and self.board[x+1][y+1] == "S" and x-1 >= 0 and y-1 >= 0 and self.board[x-1][y-1] == "S":
                #self.canvas.create_line((x+1) * 24, (y+1)*26,  (x-1) * 24, (y-1)*26, fill = c_t.get().lower(), width = 5)
                button[x][y].config(bg=w_color)
                button[x+1][y+1].config(bg=w_color)
                button[x-1][y-1].config(bg=w_color)
                return True
